...
public class PagingBean {
	 ...
	 */
	private int itemsPerPage = Constants.ITEMS_PER_PAGE;		// Constants Interface에 상수로 박아놨다
	/**
	 ...
	 * @param page 보려는 페이지.
	 */
	public PagingBean(int totalItems, int page) { //생성자로 
	 ...
	 * @return
	 */
	private int getTotalPageGroup(){	// 만약 전체수가 350이고 10으로 나누면 나누기가 0이지만 351을 10으로 나누었을때 1이 남는경우도 처리 해줘야 한다.
		
		int totalPageGoup = 0;
		int totalPage = getTotalPage();//전체 페이지 수
		if(totalPage%pagesPerPagegroup != 0){	// ==> 나누었을때 0으로 떨어지지 않는경우 
	 ...
	 * @return
	 */
	private int getCurrentPageGroup(){	// private로 만든 이유는 계산하는 과정을 눈으로 보여 줄 이유가 없기 때문에 값만 던져주면 된다.
